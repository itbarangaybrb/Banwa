...
$constructionAddress = trim($_POST['constructionAddress'] ?? '');
...
try {
    ...
    $stmt->execute([':address' => $constructionAddress]);
    ...
    if ($row) {
        ...
        echo json_encode([
            'success' => false,
            'message' => "No matching coordinates found for $constructionAddress"
        ]);
    }
} catch (Exception $e) {
    echo json_encode([
