import { initSocket } from "/client/scripts/utils/socket.js";
...
function showModal(modalId) {
  /* replaced by showSwal detail calls */
}
  ...
  if (type === "construction") {
    ...
    tableRows = [
      ...
      detailRow(
        ...
        data.construction_address || "Not specified",
      ),
      detailRow("Type of Work", data.type_of_work || "Not specified"),
      detailRow(
        "Nature of Work",
        data.nature_of_work || data.nature_of_activity || "Not specified",
      ),
      detailRow("Contractor", data.contractor_name || "Not specified"),
      detailRow(
        "Contractor Contact",
        data.contractor_contact_number || "Not specified",
      ),
      detailRow("Workers", data.number_of_workers || "0"),
      detailRow(
        "Work Period",
        `${formatDate(data.start_date)} → ${formatDate(data.end_date)} (${data.number_of_working_days || 0} days)`,
      ),
    ...
    tableRows = [
      ...
      detailRow(
        "Business Address",
        data.address_of_business || "Not specified",
      ),
      detailRow("Business Type", data.type_of_business || "Not specified"),
      detailRow(
        "Nature of Business",
        data.nature_of_business || "Not specified",
      ),
      detailRow(
        "Owner Name",
      ...
      detailRow(
        "Business Contact",
        data.telephone_no_business || "Not specified",
      ),
      ...
      detailRow(
        "Status",
        `<span style="color:${bStatusColor};font-weight:bold;">${bStatus}</span>`,
      ),
    ].join("");
    ...
    tableRows = [
      ...
      detailRow(
        "Status",
        `<span style="color:${uStatusColor};font-weight:bold;">${uStatus}</span>`,
      ),
    ].join("");
    ...
    const statusColor =
      data.status === "Resolved"
        ...
        : data.status === "Under Investigation"
          ...
          : "#cc0000";
    const dssColor =
      data.dss_status === "High Priority"
        ? "#cc0000"
        : data.dss_status === "Medium Priority"
          ? "#ff9800"
          : "#555";
    ...
    if (data.witness_data_json) {
      try {
        const witnesses =
          typeof data.witness_data_json === "string"
            ? JSON.parse(data.witness_data_json)
    ...
    tableRows = [
      ...
      detailRow(
        "Status",
        `<span style="color:${statusColor};font-weight:700;">${data.status || "Pending"}</span>`,
      ),
      detailRow(
        "Evaluation Status",
        `<span style="color:${dssColor};font-weight:700;">${data.dss_status || "Pending Evaluation"}</span>`,
      ),
      // ── Victim ──
      ...
      detailRow(
        "Full Name",
        data.vic_full_name || data.rp_full_name || "Not specified",
      ),
      ...
      detailRow(
        "Date of Birth",
        data.vic_dob ? formatDate(data.vic_dob) : "Not specified",
      ),
      ...
      detailRow(
        "Resolution Details",
        data.resolution_details || "Not specified",
      ),
      detailRow("Update Comments", data.update_comments || "Not specified"),
      data.approval_comments
        ? detailRow("Approval Comments", data.approval_comments)
        : "",
      data.disapproval_comments
        ? detailRow("Disapproval Comments", data.disapproval_comments)
        : "",
    ].join("");
...
function displayFloodDetailsInModal(data) {
  ...
  const tableRows = [
    ...
    detailRow(
      "Risk Level",
      `<span style="color:${riskColor};font-weight:bold;font-size:15px;">${(data.risk_level || "medium").toUpperCase()}</span>`,
    ),
    detailRow("Description", data.description || "Not specified"),
    detailRow(
      "Last Flood Date",
      formatDate(properties.last_flood_date) || "Not recorded",
    ),
...
function displayHouseDetailsInModal(data, apps) {
  ...
  const tableRows = [
    ...
    basicRows,
    sectionHeader("Connected Applications"),
    appRows,
  ].join("");
...
function selectFilterType(type, event) {
  ...
  if (subFilters)
    subFilters.style.display = type === "construction" ? "block" : "none";
...
function activateFilter(type) {
  ...
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("staffMapFilterChanged", { detail: { activeFilters } }),
    );
  } catch (e) {
...
function updateMarkerVisibility() {
  constructionMarkers.forEach((marker) => {
    if (activeFilter === "construction") {
      const shouldShow =
        shouldShowConstructionMarker(marker) && !marker.suppressedByHouse;
  ...
  incidentMarkers.forEach((marker) => {
    if (activeFilter === "incident") {
      const show =
        shouldShowIncidentMarker(marker) && !marker.suppressedByHouse;
    ...
    if (searchResults.length > 0) {
      ...
      topResults.forEach((result, index) => {
        ...
        const labelledFields = [
          ...
          {
            label: "Type",
            value: marker.type_of_business || marker.type_of_work,
          },
        ...
        const titleField = matchedField ||
          labelledFields[0] || { label: "", value: "Unnamed" };
        ...
        const addrField = labelledFields.find(
          (f) => f.label === "Address" || f.label === "House",
        );
        const subtitleField =
          titleField === addrField
        ...
        const highlightedSubtitle = highlightText(
          subtitleValue.substring(0, 60),
          searchTerm,
        );
...
function removeActiveSearchMarker() {
  ...
  if (activeSearchMarker) {
    ...
    if (
      activeSearchMarker.highlight &&
      map.hasLayer(activeSearchMarker.highlight)
    ) {
    ...
    if (
      activeSearchMarker instanceof L.Marker ||
...
function highlightSearchResult(markerData) {
  ...
  if (type === "construction") {
    targetMarker = constructionMarkers.find(
      (m) => m.construction_data && m.construction_data.id === markerData.id,
    );
  } else if (type === "business") {
    targetMarker = businessMarkers.find(
      (m) => m.business_data && m.business_data.id === markerData.id,
    );
  } else if (type === "utility") {
    targetMarker = utilityMarkers.find(
      (m) => m.utility_data && m.utility_data.id === markerData.id,
    );
  } else if (type === "incident") {
    targetMarker = incidentMarkers.find(
      (m) => m.incident_data && m.incident_data.id === markerData.id,
    );
  }
...
function getMarkerTypeFromData(markerData) {
  return (
    ...
    markerData.type ||
    (markerData.construction_id
      ? "construction"
      : markerData.incident_type !== undefined
        ? "incident"
        : markerData.id
          ? "business"
          : markerData.utility_id
            ? "utility"
            : markerData.hazard_id
              ? "flood"
              : markerData.house_id
                ? "household"
                : "household")
...
function highlightExistingMarker(marker, markerData) {
  ...
  pendingMoveEndHandler = () => {
    ...
    if (type === "construction")
      ...
      popupContent = createIncidentPopup(markerData);
...
function updateSearchResultActiveState(markerData) {
  document.querySelectorAll(".search-result-item").forEach((item) => {
    ...
    const resultIndex = searchResults.findIndex(
      (result) => result.marker === markerData,
    );
...
async function viewMapDetails(id, type) {
  ...
  try {
    ...
    const data = await postAction(actionMap[type] || `get_${type}_details`, {
      id,
    });
    if (data.success) displayDetailsInModal(data.data, type);
    else showErrorSwal("Error", "Could not load details.");
  } catch (e) {
    console.error("viewMapDetails:", e);
    showErrorSwal("Error", "Failed to load details.");
  }
}
...
async function viewFloodDetails(id) {
  ...
  try {
    ...
    console.error("viewFloodDetails:", e);
    showErrorSwal("Error", "Failed to load flood details.");
  }
}
...
async function viewHouseDetails(id) {
  ...
  try {
    ...
    if (data.success)
      displayHouseDetailsInModal(
        data.data,
        appData.success ? appData.applications : null,
      );
    else showErrorSwal("Error", "Could not load house details.");
  } catch (e) {
    console.error("viewHouseDetails:", e);
    showErrorSwal("Error", "Failed to load house details.");
  }
}
...
function getFloodSafetyAdvice(riskLevel) {
  ...
  return (
    advice[riskLevel] || "Take necessary precautions during heavy rainfall."
  );
}
...
async function loadAllMarkers() {
  ...
  try {
    ...
    showErrorSwal("Error Loading Markers", "Please refresh the page.");
  }
}
...
function processMarkersData(data) {
  const constructionMarkersList = (data.markers || []).filter(
    (m) => m.type === "construction",
  );
  const businessMarkersList = (data.markers || []).filter(
    (m) => m.type === "business",
  );
  const utilityMarkersList = (data.markers || []).filter(
    (m) => m.type === "utility",
  );
  const incidentMarkersList = (data.markers || []).filter(
    (m) => m.type === "incident",
  );
  ...
  constructionMarkersList.forEach((construction) => {
    ...
    marker.nature_of_work =
      construction.nature_of_work || construction.nature_of_activity;
...
async function loadFloodData() {
  try {
    ...
    console.error("ERROR LOADING FLOOD DATA:", e);
    floodLayer = L.layerGroup();
    floodLegend = null;
  }
}
...
function renderFloodAreas(hazards) {
  ...
  hazards.forEach((hazard) => {
    try {
      ...
      const paneName =
        riskPaneMap[(hazard.risk_level || "").toLowerCase()] || "floodLowPane";
...
async function loadHousePolygons() {
  try {
    ...
    if (data.success && data.houses) {
      ...
      allMarkersData = [
        ...allMarkersData,
        ...data.houses.map((h) => ({ ...h, type: "household" })),
      ];
      renderHousePolygons();
    }
  } catch (e) {
    console.error("ERROR LOADING HOUSE POLYGONS:", e);
  }
}
...
function tagMarkersWithHouseAddress() {
  function markerAddress(m) {
    if (m.construction_data)
      return (m.construction_data.construction_address || "").trim();
    if (m.business_data)
      return (m.business_data.address_of_business || "").trim();
    if (m.utility_data) return (m.utility_data.address_of_utility || "").trim();
    if (m.incident_data)
      return (
        m.incident_data.vic_address ||
        m.incident_data.rp_address ||
        ""
      ).trim();
    return "";
  }
  ...
  allAppMarkers.forEach((m) => {
    m.suppressedByHouse = false;
  });
...
async function getFloodHousesSummary() {
  try {
    ...
    const riskColors = { low: "#4caf50", medium: "#ff9800", high: "#dc3545" };
    const impactColors = {
    ...
    if (total === 0) {
      ...
      const lowCount = houses.filter(
        (h) => (h.risk_level || "").toLowerCase() === "low",
      ).length;
      const mediumCount = houses.filter(
        (h) => (h.risk_level || "").toLowerCase() === "medium",
      ).length;
      const highCount = houses.filter(
        (h) => (h.risk_level || "").toLowerCase() === "high",
      ).length;

      const fullyAffected = houses.filter(
        (h) => h.impact_level === "Fully Affected",
      ).length;
      const partiallyAffected = houses.filter(
        (h) => h.impact_level === "Partially Affected",
      ).length;
      const minimallyAffected = houses.filter(
        (h) => h.impact_level === "Minimally Affected",
      ).length;
      const affectedNoPoly = houses.filter(
        (h) => h.impact_level === "Affected",
      ).length;
      ...
      const houseRows = sortedHouses
        .map((h, i) => {
          const riskColor =
            riskColors[(h.risk_level || "").toLowerCase()] || "#888";
          const pct =
            h.impact_level === "Affected"
              ? "—"
              : parseFloat(h.flood_coverage_percent || 0).toFixed(1) + "%";
          const body = `
          ...
          return rptRow(
            ...
            h.impact_level || "?",
            riskColor,
            h.address || "Address not specified",
            `${(h.risk_level || "").toUpperCase()} · ${h.impact_level || ""}`,
            body,
          );
        })
        .join("");
      ...
      bodyHTML = `
                <div class="rpt-stats">
                    <div class="rpt-stat" style="border-left-color:#4caf50;">
                        <div class="rpt-stat-num" style="color:#2e7d32;">${lowCount}</div>
                        <div class="rpt-stat-label">Low Risk</div>
                        <div style="font-size:10px;color:#2e7d32;font-weight:600;margin-top:2px;">${total > 0 ? ((lowCount / total) * 100).toFixed(1) : "0.0"}%</div>
...
async function showFaultLineRiskAssessment() {
  showLoadingSwal(
    "Analyzing Fault Line Risk",
    "Assessing structures near the fault line...",
  );
  try {
    const result = await postAction("get_fault_line_assessment");
    if (result.status !== "success")
      throw new Error(
        result.message || "Failed to fetch fault line assessment",
      );
    ...
    const maxDist = Math.max(
      ...(data.structures || []).map((s) => s.distance_meters),
      350,
    );
    ...
    if (!data.structures || data.structures.length === 0) {
      ...
      const sorted = [...(data.structures || [])].sort(
...
async function showAllBusinessesSDSSReport() {
  showLoadingSwal(
    "Generating Report",
    "Analyzing business safety compliance...",
  );
  try {
    ...
    const data = result.data;
    if (!data?.summary || !data?.warnings)
      throw new Error("Invalid data structure received from server");
    displayBusinessSDSSReport(data);
  } catch (e) {
    console.error("Error in business SDSS:", e);
    showErrorSwal("Error Generating Report", e.message);
  }
}
...
async function showAllConstructionSDSSReport() {
  ...
  try {
    ...
    if (result.status !== "success")
      throw new Error(
        result.message || "Failed to fetch construction SDSS report",
      );
    const data = result.data;
    if (!data?.summary || !data?.warnings)
      throw new Error("Invalid data structure received from server");
    ...
    console.error("Error in construction SDSS:", e);
    showErrorSwal("Error Generating Report", e.message);
  }
}
...
function displayBusinessSDSSReport(data) {
  ...
  const critical = warnings.filter((w) =>
    w.warnings?.some((x) => x.severity === "CRITICAL"),
  );
  const high = warnings.filter(
    (w) =>
      w.warnings?.some((x) => x.severity === "HIGH") && !critical.includes(w),
  );
  const medium = warnings.filter(
    (w) =>
      w.warnings?.some((x) => x.severity === "MEDIUM") &&
      !critical.includes(w) &&
      !high.includes(w),
  );
  ...
  const rows = sorted
    .map((item, idx) => {
      ...
      const c =
        sev === "CRITICAL" ? "#990000" : sev === "HIGH" ? "#cc0000" : "#ff9800";
...
async function loadIncidentSubFilters() {
  ...
  const typeButtons = types
    .map((type) => {
      ...
      const icon =
        Object.entries(typeIcons).find(([k]) => key.includes(k))?.[1] ||
  ...
  const currentLabel =
    incidentSubFilter === "all" ? "All types" : incidentSubFilter;
...
function filterIncidentByType(subtype, event) {
  ...
  document
    .querySelectorAll("[data-incident-subtype]")
    .forEach((btn) => btn.classList.remove("active"));
  const clickedBtn = event
    ? event.currentTarget
  ...
  if (label)
    label.innerHTML =
      subtype === "all" ? "Showing all types" : `<strong>${subtype}</strong>`;
...
function shouldShowIncidentMarker(marker) {
  ...
  const type = (
    ...
    (marker.incident_data ? marker.incident_data.incident_type : "") ||
    ""
  ).toLowerCase();
...
async function showIncidentSummaryReport() {
  ...
  try {
    ...
    showSwal({
      icon: "error",
      title: "Error",
      text: "Failed to generate incident report: " + error.message,
    });
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (searchInput) {
    ...
    document.addEventListener("click", function (e) {
      const resultsContainer = document.getElementById("search-results");
      const searchBox =
        document.querySelector(".gm-search-box") ||
        document.querySelector(".map-overlay--search");
  ...
  const householdLink = document.querySelector(
    '.dropdown-content a[data-type="household"]',
  );
  if (householdLink) {
...
map.whenReady(async function () {
  ...
  try {
    ...
    console.warn("Init data fetch failed:", e);
  }

  const boundaryData = initData;
  const markersData = initData;
  const housesData = initData
    ? { success: initData.success, houses: initData.houses }
    : null;
  ...
  try {
    ...
    console.warn("Could not parse boundary:", e);
  }
  ...
  faultLine.on("mouseover", function () {
    this.setStyle({ weight: 6, opacity: 1, color: "#cc0000" });
  });
  faultLine.on("mouseout", function () {
    this.setStyle({ weight: 4, opacity: 0.8, color: "#ff0000" });
  });
  ...
  if (housesData?.success && housesData.houses) {
    ...
    const housesForSearch = housesData.houses.map((h) => ({
      ...h,
      type: "household",
    }));
...
function setupSoftBoundary() {
    ...
    if (!warningBounds.contains(currentCenter)) {
      ...
      if (!softBounds.contains(currentCenter)) {
        boundaryTimeout = setTimeout(function () {
          ...
          if (snappedLat > warningBounds.getNorth())
            snappedLat = warningBounds.getNorth();
          if (snappedLat < warningBounds.getSouth())
            snappedLat = warningBounds.getSouth();
          if (snappedLng > warningBounds.getEast())
            snappedLng = warningBounds.getEast();
          if (snappedLng < warningBounds.getWest())
            snappedLng = warningBounds.getWest();
  ...
  map.on("moveend", function () {
    ...
    if (!softBounds.contains(currentCenter)) {
      ...
      if (snappedLat > softBounds.getNorth())
        snappedLat = softBounds.getNorth();
      if (snappedLat < softBounds.getSouth())
        snappedLat = softBounds.getSouth();
...
function debugFloodState() {
  ...
  console.log(
    "Flood Layer on map:",
    floodLayer ? map.hasLayer(floodLayer) : false,
  );
  console.log("Flood Legend exists:", !!floodLegend);
  console.log(
    "Flood Legend on map:",
    floodLegend ? hasControl(floodLegend) : false,
  );
...
async function showSDSSRulesReport() {
  showLoadingSwal(
    "Loading Barangay Rules",
    "Please wait while we load the rules summary...",
  );
  try {
    ...
    if (rulesResult.status === "success")
      displaySDSSRulesReport(
        rulesResult.data,
        dssResult.success ? dssResult : null,
      );
    else
      showErrorSwal(
        "Error",
        rulesResult.message || "Failed to load rules summary",
      );
  } catch (e) {
    console.error("showSDSSRulesReport:", e);
    showErrorSwal("Error", "Failed to fetch Barangay Rules");
  }
}
...
function displaySDSSRulesReport(data, dssData) {
  ...
  for (const [key, rule] of Object.entries(rules)) {
    ...
    else if (rule.category === "Construction Safety")
      constructionRules.push({ key, ...rule });
    else if (rule.category === "Business Rules")
      businessRules.push({ key, ...rule });
    else if (rule.category === "Incident Rules")
      incidentRules.push({ key, ...rule });
  }
    ...
    didOpen: (popup) => {
      ...
      popup.addEventListener("click", (e) => {
        ...
        if (ruleBtn) {
          ...
          setTimeout(
            () =>
              showRuleAffectedData(
                ruleBtn.dataset.ruleKey,
                ruleBtn.dataset.ruleName,
                true,
              ),
            200,
          );
        ...
        if (bulkBtn) {
          ...
          const evList = (window._dssEvals || []).filter((_, i) =>
            indices.includes(i),
          );
          if (evList.length > 0) {
            Swal.close();
...
function switchRulesTab(tab) {
  document.getElementById("tab-rules").style.display =
    tab === "rules" ? "" : "none";
  document.getElementById("tab-dss").style.display =
    tab === "dss" ? "" : "none";
...
function buildDSSTab(dssData) {
  ...
  const statusColor = (s) =>
    s === "Pre-Approved" ? "#28a745" : s === "Rejected" ? "#dc3545" : "#ff9800";
  const typeIcon = (t) =>
    t === "construction"
      ? "fa-hard-hat"
      : t === "business"
  ...
  const statusRank = {
    "Pre-Approved": 1,
    "Additional Requirements Needed": 2,
    ...
    .map(([type, items]) => {
      ...
      const rows = items
        .map((ev, i) => {
          ...
          const pct =
            ev.max_score > 0 ? Math.round((ev.score / ev.max_score) * 100) : 0;
...
function createRuleCard(rule, totalHouses, cardId) {
  ...
  const pct =
    totalHouses > 0 ? ((rule.count / totalHouses) * 100).toFixed(1) : "0.0";
...
function clearAffectedHighlights() {
  affectedHighlightLayers.forEach((l) => {
...
async function showRuleAffectedData(ruleKey, ruleName, fromSDSS = false) {
  ...
  try {
    const result = await postAction("get_rule_affected_data", {
      rule_key: ruleKey,
    });
    ...
    records.forEach((r) => {
      if (r.type === "household") {
        const houseData = housePolygonsData.find(
          (h) => String(h.house_id) === String(r.id),
        );
        if (houseData && houseData.coordinates) {
          try {
            ...
            poly.bindPopup(
              `<div class="popup-content"><h4><span>${r.name}</span><span class="household-badge" style="background:#dc3545;">Affected</span></h4><div class="popup-section"><p><strong>Address:</strong> ${r.address}</p><p><strong>Detail:</strong> ${r.detail}</p></div></div>`,
            );
            affectedHighlightLayers.push(poly);
            return;
          } catch (e) {
            console.warn("Polygon parse error", e);
          }
        }
        const lat = parseFloat(r.lat || r.center_lat),
          lng = parseFloat(r.lng || r.center_lng);
        if (!isNaN(lat) && !isNaN(lng)) {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
          ...
          const nearbyHouse = housePolygonsData.find((h) => {
            const hLat = parseFloat(h.center_lat),
              hLng = parseFloat(h.center_lng);
          ...
          if (nearbyHouse && nearbyHouse.coordinates) {
            try {
              ...
              latLngs.push(latLngs[0]);
              const badgeClass =
                r.type === "business" ? "business-badge" : "construction-badge";
              const poly = L.polygon(latLngs, {
                color: "#dc3545",
                weight: 3,
                fillColor: "#dc3545",
              ...
              poly.bindPopup(
                `<div class="popup-content"><h4><span>${r.name}</span><span class="${badgeClass}" style="background:#dc3545;">Affected</span></h4><div class="popup-section"><p><strong>Address:</strong> ${r.address}</p><p><strong>Detail:</strong> ${r.detail}</p></div></div>`,
              );
              affectedHighlightLayers.push(poly);
              return;
            } catch (e) {
              console.warn("Polygon parse error for non-household:", e);
            }
          }
...
`;
    ...
    const bodyEl = document.getElementById("affected-panel-body");
    if (bodyEl)
      bodyEl.innerHTML = `<div style="padding:20px;color:#cc0000;font-size:13px;">Failed to fetch affected records.</div>`;
  }
}
...
async function showDSSEvalsOnMap(type, evList) {
  ...
  document.addEventListener("keydown", function escHDss(e) {
    if (e.key === "Escape") {
      closePanel();
      document.removeEventListener("keydown", escHDss);
    }
  });
  ...
  panelRows.sort(
    (a, b) =>
      (panelStatusRank[a.ev.dss_status] || 9) -
      (panelStatusRank[b.ev.dss_status] || 9),
  );
  panelRows.forEach(({ ev, sc, pct, marker }) => {
    ...
    div.addEventListener("click", () => {
      listEl
        .querySelectorAll(".affected-record-row")
...
const messageMap = {
  ...
  incident_report_applications_update:
    "New incident report application added to map",
