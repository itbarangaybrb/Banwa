...
async function handleSubmitChanges(event, appId) {
    ...
    try {
        ...
        for (const key in originalData) {
            ...
            if ((mappedKey === 'businessStatus' || mappedKey === 'requirements') && Array.isArray(originalData[key])) {
                 originalData[key].forEach(item => finalFormData.append(`${mappedKey}[]`, item));
            } else if (originalData[key] !== null) {
