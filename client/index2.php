...
function openModal(){if(!modal)return;modal.style.display='flex';requestAnimationFrame(function(){modal.classList.add('show');});}
['navGetStartedBtn'].forEach(function(id){var b=document.getElementById(id);if(b)b.addEventListener('click',openModal);});

/* HERO CAROUSEL */
var hSlides=document.querySelectorAll('.hero__bg-slide');
var hDots=document.querySelectorAll('.hero__dot');
var hIdx=0;
function goHero(i){hSlides[hIdx].classList.remove('is-active');hDots[hIdx].classList.remove('is-active');hIdx=(i+hSlides.length)%hSlides.length;hSlides[hIdx].classList.add('is-active');hDots[hIdx].classList.add('is-active');}
hDots.forEach(function(d){d.addEventListener('click',function(){goHero(+this.dataset.slide);});});
setInterval(function(){goHero(hIdx+1);},5500);
