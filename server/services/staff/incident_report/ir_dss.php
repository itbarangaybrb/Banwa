 ...
 * Fetches report data, runs through DSS rule engine, and updates evaluation records
 * 
...
{
    try {
        // SELECT from incident_reports table (not incident_report_evaluations)
        ...
        // Check if evaluation already exists in incident_report_evaluations
        ...
        // Update dss_status in incident_reports table (not incident_report_evaluations)
 ...
 * This ensures all reports have a DSS evaluation record even before evaluation
 * 
 * @param PDO $pdo Database connection object
...
function createInitialDSSEvaluation($pdo, $reportId)
{
    try {
        // SELECT from incident_reports table (not incident_report_evaluations)
        $stmt = $pdo->prepare("SELECT * FROM incident_reports WHERE id = :application_id");
        ...
        // Insert into incident_report_evaluations table
        $sql = "INSERT INTO incident_report_evaluations 
                (application_id, dss_status, evaluation_details, evaluated_at) 
                VALUES (:application_id, :status, :details, NOW())";
        ...
        // Update dss_status in incident_reports table
        ...
        // Log the evaluation
        $logFile = __DIR__ . '/dss_incident_report_evaluations.log';
        $logEntry = [
            'timestamp' => date('Y-m-d H:i:s'),
            'application_id' => $reportId,
            'dss_status' => $evaluationResult['status'],
        ...
        // Insert error record into incident_report_evaluations
        $sql = "INSERT INTO incident_report_evaluations 
                (application_id, dss_status, evaluation_details, evaluated_at) 
                VALUES (:application_id, 'Evaluation Error', '{}', NOW())";
 ...
 * Provides score, priority level, urgency score, and rule pass/fail counts for frontend display
 * 
 * @param array $evaluationDetails DSS evaluation details array
 * @return array Summary statistics including score, priority level, and counts
 */
